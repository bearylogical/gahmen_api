...
func (s *APIServer) Run() {
	...
	authRouter.HandleFunc("GET /api/v1/documents/{document_id}", makeHttpHandleFunc(handler.GetDocumentByID))
	authRouter.HandleFunc("GET /api/v1/documents/ministry/{ministry_id}", makeHttpHandleFunc(handler.ListDocumentByMinistryID))
	authRouter.HandleFunc("GET /api/v1/expenditures", makeHttpHandleFunc(handler.ListExpenditure))
	authRouter.HandleFunc("GET /api/v1/projects/{project_id}", makeHttpHandleFunc(handler.GetProjectExpenditureByID))
	authRouter.HandleFunc("GET /api/v1/programmes/ministry/{ministry_id}", makeHttpHandleFunc(handler.GetProgrammeExpenditureByMinistryID))
	authRouter.HandleFunc("GET /api/v1/budget/options", makeHttpHandleFunc(handler.GetBudgetOpts))
	authRouter.HandleFunc("GET /api/v1/expenditures/ministry/{ministry_id}", makeHttpHandleFunc(handler.ListExpenditureByMinistry))
	authRouter.HandleFunc("GET /api/v1/projects/ministry/{ministry_id}", makeHttpHandleFunc(handler.ListProjectByMinistryID))
	authRouter.HandleFunc("GET /api/v1/projects/expenditure/ministry/{ministry_id}", makeHttpHandleFunc(handler.ListProjectExpenditureByMinistryID))
	authRouter.HandleFunc("GET /api/v1/sgdi-links/ministry/{ministry_id}", makeHttpHandleFunc(handler.ListSGDILinksByMinistryID))
	authRouter.HandleFunc("GET /api/v1/personnel/{ministryID}", makeHttpHandleFunc(handler.ListTopNPersonnelByMinistryID))
	authRouter.HandleFunc("GET /api/v2/budget", makeHttpHandleFunc(handler.GetMinistryDataV2))
	authRouter.HandleFunc("POST /api/v1/projects", makeHttpHandleFunc(handler.GetProjectExpenditureByQuery))
